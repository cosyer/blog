...
try {
  ...
  window.location.href = "https://www.baidu.com/s?wd=" + e.message;
}
```
...
字符串比较大小是按照字符串从左到右每个字符的 charCode 来的，但所以特别要注意时间形式注意补 0
...
## Math.round 实现的精确保留指定位数的函数(toFixed 不完善)
